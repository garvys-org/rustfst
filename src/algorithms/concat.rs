    ...
    F3: MutableFst<W=W>,
{
	...
	fst_out.set_start(mapping_states_fst_1.get(&old_start_state).unwrap());
	...
	let start_state_2 = mapping_states_fst_2.get(&old_start_state_2).unwrap();
	for old_final_state_1 in fst_1.final_states_iter() {
		let final_state_1 = mapping_states_fst_1.get(&old_final_state_1).unwrap();
