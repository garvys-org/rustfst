...
#[derive(Debug)]
pub struct LruFstCache<W: Semiring> {
    ...
    trs: Mutex<(lru::LruCache<StateId, CacheTrs<W>>, usize)>,
    final_weights: Mutex<(lru::LruCache<StateId, Option<W>>, usize)>,
}

impl<W: Semiring> Default for LruFstCache<W> {
    fn default() -> Self {
        Self {
...
impl<W: Semiring> FstCache<W> for LruFstCache<W> {
