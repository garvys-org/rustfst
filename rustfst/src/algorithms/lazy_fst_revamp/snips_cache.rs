...
impl<W: Semiring> PreInitializedCache<W> {
    pub fn mem_size(&self) -> usize {
        ...
        let data_trs = self.trs.lock().unwrap();

        for (_, trs) in data_trs.0.iter() {
            mem_size += std::mem::size_of::<StateId>() + trs.len() * std::mem::size_of::<Tr<W>>()
        }

        drop(data_trs);

        let data_final_weight = self.final_weight.lock().unwrap();
        mem_size +=
            (std::mem::size_of::<StateId>() + std::mem::size_of::<W>()) * data_final_weight.0.len();
