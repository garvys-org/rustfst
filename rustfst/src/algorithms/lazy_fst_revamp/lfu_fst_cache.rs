...
#[derive(Debug)]
pub struct LfuFstCache<W: Semiring> {
    ...
    trs: Mutex<(lfu::LFUCache<StateId, CacheTrs<W>>, usize)>,
    final_weights: Mutex<(lfu::LFUCache<StateId, Option<W>>, usize)>,
}
...
impl<W: Semiring> LfuFstCache<W> {
    pub fn new(cap_trs: usize, cap_final_weights: usize) -> Self {
        Self {
            start: Mutex::new((None, 0)),
            trs: Mutex::new((lfu::LFUCache::with_capacity(cap_trs).unwrap(), 0)),
            final_weights: Mutex::new((lfu::LFUCache::with_capacity(cap_final_weights).unwrap(), 0)),
        }
    }
}

impl<W: Semiring> FstCache<W> for LfuFstCache<W> {
