...
#[derive(Debug, Clone)]
pub struct NullComposeFilter<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
...
#[derive(Debug)]
pub struct NullComposeFilterBuilder<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
...
impl<W, F1, F2, M1, M2> Clone for NullComposeFilterBuilder<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
...
impl<W: Semiring, F1, F2, M1, M2> ComposeFilterBuilder<W, F1, F2, M1, M2>
    for NullComposeFilterBuilder<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
    ...
    M2: Matcher<W, F2>,
{
    type IM1 = M1;
    type IM2 = M2;
    type CF = NullComposeFilter<W, F1, F2, M1, M2>;
    ...
    fn build(&self) -> Result<Self::CF> {
        Ok(NullComposeFilter::<W, F1, F2, M1, M2> {
...
impl<W, F1, F2, M1, M2> ComposeFilter<W, F1, F2, M1, M2> for NullComposeFilter<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
