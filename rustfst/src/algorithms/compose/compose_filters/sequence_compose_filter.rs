...
pub struct SequenceComposeFilter<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
...
#[derive(Debug)]
pub struct SequenceComposeFilterBuilder<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
...
impl<W, F1, F2, M1, M2> Clone for SequenceComposeFilterBuilder<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
...
impl<W, F1, F2, M1, M2> ComposeFilterBuilder<W, F1, F2, M1, M2>
    for SequenceComposeFilterBuilder<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
    ...
    M2: Matcher<W, F2>,
{
    type IM1 = M1;
    type IM2 = M2;
    type CF = SequenceComposeFilter<W, F1, F2, M1, M2>;
    ...
    fn build(&self) -> Result<Self::CF> {
        Ok(SequenceComposeFilter::<W, F1, F2, M1, M2> {
...
impl<W, F1, F2, M1, M2> ComposeFilter<W, F1, F2, M1, M2>
    for SequenceComposeFilter<W, F1, F2, M1, M2>
where
    W: Semiring,
    F1: Fst<W>,
    F2: Fst<W>,
