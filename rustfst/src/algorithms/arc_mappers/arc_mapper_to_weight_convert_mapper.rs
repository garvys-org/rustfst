macro_rules! arc_mapper_to_weight_convert_mapper_methods {
    ($semiring: ty) => {
        fn arc_map(&mut self, arc: &Arc<$semiring>) -> Result<Arc<$semiring>> {
            let mut mapped_arc = arc.clone();
            (self as &mut dyn ArcMapper<$semiring>).arc_map(&mut mapped_arc)?;
            Ok(mapped_arc)
        }

        fn final_arc_map(&mut self, final_arc: &FinalArc<$semiring>) -> Result<FinalArc<$semiring>> {
