...
use rustfst::fst_traits::{
    Fst, FstIterData, FstIterator, MutableFst, SerializableFst, StateIterator,
};
use rustfst::semirings::TropicalWeight;
use rustfst::Semiring;
use rustfst::{DrawingConfig, StateId, TrsVec};
use std::ffi::CStr;

//type CVecFst = CFst;
//type CConcatFst = CFst<ConcatFst<TropicalWeight, VectorFst<TropicalWeight>>>;

type BoxedFst = Box<
    dyn Fst<
        TropicalWeight,
        TRS = TrsVec<TropicalWeight>,
        Iter = dyn for<'a> StateIterator<'a, Iter = dyn Iterator<Item = StateId> + 'a>,
        FstIter = dyn for<'b> FstIterator<
            'b,
            TropicalWeight,
            TRS = TrsVec<TropicalWeight>,
            FstIter = dyn Iterator<Item = FstIterData<TropicalWeight, TrsVec<TropicalWeight>>> + 'b,
        >,
    >,
>;

#[derive(RawPointerConverter)]
pub struct CFst(pub(crate) BoxedFst);
